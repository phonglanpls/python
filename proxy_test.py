...
def handler_https(first_line, header, sock, source_data, last_ws):
    # print first_line
    # print header
    # print sock
    ...
    # 尝试连接
    try:
        # print (socket.gethostbyname(first_line[2]), first_line[4])
        ...
        try:
            ...
            while True:
                # 从客户端读取数据，并转发给conn
                try:
                    send_to_server += last_ws.recv(999999)
                except:
                    pass

                soc.send(send_to_server)
                # 从服务器读取回复，转发回客户端
                receive = soc.recv(999999)
                print '|' + receive + '|'
                # return receive
...
User-Agent: Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36
    ...
    if first_line[0] == 'CONNECT':  # HTTPS
        return handler_https(first_line, header, sock, data, last_ws)
